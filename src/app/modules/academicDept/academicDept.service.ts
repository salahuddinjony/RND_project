...
const createAcademicDeptIntoDB = async (academicDeptData: AcademicDept) => {
  const academicFacultyId = academicDeptData.academicFaculty.toString();
  if (!academicFacultyId) {
    throw new AppError("Academic faculty is required", 400);
  }
  if (!mongoose.Types.ObjectId.isValid(academicFacultyId)) {
    throw new AppError("Invalid academic faculty id", 400);
  }
  //check if the academic faculty is deleted
  const academicFaculty = await AcademicFacultyModel.findOne({
    _id: academicFacultyId,
    isDeleted: false,
  });
  if (!academicFaculty) {
    throw new AppError("Academic faculty is deleted", 400);
  }
...
const getAllAcademicDeptsFromDB = async (
  ...
  const { meta, data: academicDepts } = await paginate(
    AcademicDeptModel,
    parsed,
    (q) => q.populate("academicFaculty"),
  );
  return {
    total: meta.total,
    page: meta.page,
...
const getAcademicDeptByIdFromDB = async (id: string) => {
  const academicDept =
    await AcademicDeptModel.findById(id).populate("academicFaculty");
  return academicDept;
};

// update department info
const updateAcademicDeptInfoInDB = async (
  id: string,
  updatedData: Partial<Omit<AcademicDept, "id">>,
) => {
  ...
  const existingDept = await AcademicDeptModel.findOne({
    _id: id,
    isDeleted: false,
  });
  if (!existingDept) {
...
const getAllDeletedAcademicDeptsFromDB = async () => {
  const academicDepts = await AcademicDeptModel.find({
    isDeleted: true,
  }).populate("academicFaculty");
...
const restoreDeletedAcademicDeptsInDB = async () => {
  ...
  const deletedDepts = await AcademicDeptModel.find({ isDeleted: true }).select(
    "_id",
  ); // We only need the _id field of the deleted departments to restore them, so we can use select to optimize the query and reduce the amount of data retrieved from the database
  ...
  const latestRestoredDepts = await AcademicDeptModel.find({
    isDeleted: false,
    _id: { $in: deletedDepts },
  }).select("name academicFaculty updatedAt");
