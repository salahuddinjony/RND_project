...
const getAllSemestersFromDB = async (query: Record<string, unknown> = {}) => {
  const parsed = parseListQuery(query, {
    searchableFields: ["name", "code", "startMonth", "endMonth"],
    baseFilter: { isDeleted: false },
  });
  const { meta, data } = await paginate(AcademicSemesterModel, parsed);
  return { meta, data };
};
...
const updateSemesterInfoInDB = async (
  id: string,
  updatedData: Partial<Omit<AcademicSemester, "id">>,
) => {
...
const restoreDeletedSemestersInDB = async () => {
  ...
  const deletedSemesters = await AcademicSemesterModel.find({
    isDeleted: true,
  });
  ...
  const latestRestoredSemesters = await AcademicSemesterModel.find({
    isDeleted: false,
  }).sort({ updatedAt: -1 });
...
const restoreDeletedSemesterByIdInDB = async (id: string) => {
  const restoredSemester = await AcademicSemesterModel.findByIdAndUpdate(
    id,
    { isDeleted: false },
    { returnDocument: "after" },
  );
